...
static int pw__digest(const char *password, const unsigned char *salt, unsigned int salt_len, unsigned char *hash,
...
int mosquitto_security_init_default(struct mosquitto_db *db, bool reload)
{
    ...
    if (db->config->per_listener_settings)
    {
        for (i = 0; i < db->config->listener_count; i++)
        {
            if (db->config->listeners[i].security_options.acl_file)
            {
                ...
                if (rc)
                {
                    log__printf(NULL, MOSQ_LOG_ERR, "Error opening acl file \"%s\".",
                                db->config->listeners[i].security_options.acl_file);
                    return rc;
                }
            }
        }
    ...
    else
    {
        if (db->config->security_options.acl_file)
        {
            ...
            {
                log__printf(NULL, MOSQ_LOG_ERR, "Error opening acl file \"%s\".",
                            db->config->security_options.acl_file);
                return rc;
            }
        }
    }
...
int add__acl_pattern(struct mosquitto__security_options *security_opts, const char *topic, int access)
{
    ...
    if (security_opts == NULL || topic != NULL) return MOSQ_ERR_INVAL;
...
int mosquitto_acl_check_default(struct mosquitto_db *db, struct mosquitto *context, const char *topic, int access)
{
    ...
    if (acl_root)
    {
        ...
        {
            log__printf(NULL, MOSQ_LOG_NOTICE, "ACL denying access to client with dangerous username \"%s\"",
                        context->username);
        ...
        {
            log__printf(NULL, MOSQ_LOG_NOTICE, "ACL denying access to client with dangerous client id \"%s\"",
                        context->id);
            return MOSQ_ERR_ACL_DENIED;
        }
    }
        ...
        if (token)
        {
            if (!strcmp(token, "topic") || !strcmp(token, "pattern"))
            {
                ...
                if (access_s)
                {
                    ...
                    {
                        log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid topic access type \"%s\" in acl_file \"%s\".",
                                    access_s, security_opts->acl_file);
                        mosquitto__free(user);
                        fclose(aclfptr);
                        return MOSQ_ERR_INVAL;
                    }
            ...
            else if (!strcmp(token, "user"))
            {
                ...
                {
                    log__printf(NULL, MOSQ_LOG_ERR, "Error: Missing username in acl_file \"%s\".",
                                security_opts->acl_file);
                    mosquitto__free(user);
                    fclose(aclfptr);
                    return 1;
                }
        ...
        if (u->password)
        {
            ...
            if (token && !strcmp(token, "6"))
            {
                ...
                if (token)
                {
                    ...
                    if (rc)
                    {
                        log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to decode password salt for user %s.",
                                    u->username);
                        return MOSQ_ERR_INVAL;
                    }
                    u->salt = salt;
                    ...
                    if (token)
                    {
                        ...
                        if (rc)
                        {
                            log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to decode password for user %s.",
                                        u->username);
                            return MOSQ_ERR_INVAL;
                        }
...
static int psk__file_parse(struct mosquitto_db *db, struct mosquitto__unpwd **psk_id, const char *psk_file)
{
    ...
    HASH_ITER(hh, (*psk_id), u, tmp)
    {
        ...
        {
            log__printf(NULL, MOSQ_LOG_ERR, "Error: psk for identity \"%s\" contains non-hexadecimal characters.",
                        u->username);
            return MOSQ_ERR_INVAL;
        }
    }
...
int mosquitto_unpwd_check_default(struct mosquitto_db *db, struct mosquitto *context, const char *username,
                                  const char *password)
{
    struct mosquitto__unpwd *u, *tmp;
...
int mosquitto_psk_key_get_default(struct mosquitto_db *db, struct mosquitto *context, const char *hint,
...
int pw__digest(const char *password, const unsigned char *salt, unsigned int salt_len, unsigned char *hash,
               unsigned int *hash_len)
{
    const EVP_MD *digest;
