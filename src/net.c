...
int net__socket_accept(struct mosquitto_db *db, mosq_sock_t listensock)
{
    ...
    {
        log__printf(NULL, MOSQ_LOG_NOTICE, "Client connection from %s denied: max_connections exceeded.",
                    new_context->address);
        context__cleanup(db, new_context, true);
        return -1;
    }
    ...
    log__printf(NULL, MOSQ_LOG_NOTICE, "New connection from %s on port %d.", new_context->address,
                new_context->listener->port);
...
static int mosquitto__tls_server_ctx(struct mosquitto__listener *listener)
{
    ...
    if (listener->ciphers)
    {
        ...
        if (rc == 0)
        {
            log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to set TLS ciphers. Check cipher list \"%s\".",
                        listener->ciphers);
            return 1;
        }
    }
...
int net__socket_listen(struct mosquitto__listener *listener)
{
    ...
    for (rp = ainfo; rp; rp = rp->ai_next)
    {
        ...
        {
            log__printf(NULL, MOSQ_LOG_INFO, "Opening ipv4 listen socket on port %d.",
                        ntohs(((struct sockaddr_in *) rp->ai_addr)->sin_port));
        }
        else if (rp->ai_family == AF_INET6)
        {
            log__printf(NULL, MOSQ_LOG_INFO, "Opening ipv6 listen socket on port %d.",
    ...
    if (listener->sock_count > 0)
    {
        ...
        if ((listener->cafile || listener->capath) && listener->certfile && listener->keyfile)
        {
            ...
            if (rc == 0)
            {
                ...
                {
                    log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to load CA certificates. Check cafile \"%s\".",
                                listener->cafile);
                }
                else
                {
                    log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to load CA certificates. Check capath \"%s\".",
                                listener->capath);
                }
                net__print_error(MOSQ_LOG_ERR, "Error: %s");
                COMPAT_CLOSE(sock);
            ...
            {
                SSL_CTX_set_verify(listener->ssl_ctx, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT,
            ...
            if (rc != 1)
            {
                log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to load server certificate \"%s\". Check certfile.",
                            listener->certfile);
                net__print_error(MOSQ_LOG_ERR, "Error: %s");
                COMPAT_CLOSE(sock);
                return 1;
            ...
            if (rc != 1)
            {
                log__printf(NULL, MOSQ_LOG_ERR, "Error: Unable to load server key file \"%s\". Check keyfile.",
                            listener->keyfile);
                net__print_error(MOSQ_LOG_ERR, "Error: %s");
                COMPAT_CLOSE(sock);
                return 1;
            ...
            if (listener->crlfile)
            {
                ...
                {
                    log__printf(NULL, MOSQ_LOG_ERR,
                                "Error: Unable to load certificate revocation file \"%s\". Check crlfile.",
                                listener->crlfile);
                    net__print_error(MOSQ_LOG_ERR, "Error: %s");
                    COMPAT_CLOSE(sock);
                    return 1;
